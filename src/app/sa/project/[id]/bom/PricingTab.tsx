...
export function PricingTab({ state, selectedSite }: { state: BOMBuilderState, selectedSite: Site }) {
    ...
    // Filter items for just this site
    const rawSiteItems = bom.items.filter(
        i => i.siteName === selectedSite.name && SITE_TAB_SERVICE_IDS.has(i.serviceId)
    );
    ...
    siteItems.forEach(item => {
        ...
        siteTotalNet += netPrice * item.quantity;
    });
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                {siteItems.length === 0 ? (
                    <div className="p-8 text-center text-sm text-slate-500">
                        No pricing data available for this site&apos;s current configuration.
                    </div>
                ) : (
