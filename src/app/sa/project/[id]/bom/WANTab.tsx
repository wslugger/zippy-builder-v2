...
export function WANTab({
    ...
    // Per-circuit monthly cost — stored locally per site name
    const [circuitCosts, setCircuitCosts] = useState<Record<string, string>>({});

    const primaryKey = `${selectedSite.name}:primary`;
    const secondaryKey = `${selectedSite.name}:secondary`;

    const setCost = (key: string, value: string) =>
        setCircuitCosts(prev => ({ ...prev, [key]: value }));
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">Circuit Pricing</span>
                </div>
                <div className="space-y-3">
                    {/* Primary circuit row */}
                    <div className="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800/50">
                        <div className="bg-slate-100 dark:bg-slate-800 p-2 rounded-full flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-slate-900 dark:text-slate-100 flex items-center gap-2">
                                ...
                                <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 px-1.5 py-0.5 rounded uppercase font-bold">PRI</span>
                            </div>
                            <div className="text-xs text-slate-500 dark:text-slate-400">Partner ISP • {selectedSite.bandwidthDownMbps} Mbps</div>
                        </div>
                        {/* Monthly cost input */}
                        <div className="flex items-center gap-1 flex-shrink-0">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    min="0"
                                    placeholder="—"
                                    value={circuitCosts[primaryKey] ?? ""}
                                    onChange={e => setCost(primaryKey, e.target.value)}
                                    className="w-28 pl-5 pr-2 py-1.5 text-sm text-right font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-slate-900 dark:text-slate-100"
                                />
                            </div>
                        </div>
                    </div>

                    {/* Secondary circuit row */}
                    {selectedSite.secondaryCircuit && (
                        <div className="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800/50">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-sm font-medium text-slate-900 dark:text-slate-100 flex items-center gap-2">
                                    {selectedSite.secondaryCircuit}
                                    <span className="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400 px-1.5 py-0.5 rounded uppercase font-bold">SEC</span>
                                </div>
                                <div className="text-xs text-slate-500 dark:text-slate-400">Local Cable • {Math.round(selectedSite.bandwidthDownMbps * 0.1)} Mbps</div>
                            </div>
                            {/* Monthly cost input */}
                            <div className="flex items-center gap-1 flex-shrink-0">
                                <span className="text-xs text-slate-400 font-medium">$/mo</span>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        min="0"
                                        placeholder="—"
                                        value={circuitCosts[secondaryKey] ?? ""}
                                        onChange={e => setCost(secondaryKey, e.target.value)}
                                        className="w-28 pl-5 pr-2 py-1.5 text-sm text-right font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-slate-900 dark:text-slate-100"
                                    />
                                </div>
                            </div>
                    ...
                    {/* Monthly circuit total pill */}
                    {(circuitCosts[primaryKey] || circuitCosts[secondaryKey]) && (() => {
                        const pri = parseFloat(circuitCosts[primaryKey] || "0") || 0;
                        const sec = parseFloat(circuitCosts[secondaryKey] || "0") || 0;
                        const total = pri + sec;
                        return (
                            <div className="flex justify-end pt-1">
                                <div className="inline-flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg px-3 py-2">
                                    <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">Circuit Total / mo</span>
